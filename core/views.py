...
from .models import LikePost, Profile, Post, FollowerCount, Room, Message
...
def index(request):
    ...
    for username in user_following_list:
        feed_list_local = Post.objects.filter(user=username)
        feed.append(feed_list_local)
    ...
    if request.method == 'POST':
        ...
        if request.FILES.get('image') == None:
            if request.FILES.get('image2') == None:
                ...
                image2 = user_profile.featureimg
                bio = request.POST['profile_bio']
                location = request.POST['location']

                user_profile.profileimg = image
                user_profile.featureimg = image2
